 ...
 {            // if we choose penalty kick
             ...
             // bot is below the ball, NOT perpendicular to the field
                ...
                if (fabs(ai->st.self->cx[0] - ai->st.ball->cx[0]) < 200 ){
                  ...
                  fprintf(stderr, "off angle from center %f\n", fabs(angle));
                  fprintf(stderr, "perfect_kick value%d\n", left_right);
                  if (fabs(angle) > 10){   //if angle is off + or - 10 degrees,
                    fprintf(stderr, "\n######## changing angle w#####\n");
                    if ((flag%3) == 0){
                      fprintf(stderr, "         %d       \n", flag);
...
void turn_bot(int left_right, double angle_out){
  double error = angle_out - 5.0;
  if (left_right){
    turn_left_speed(4* (int)error);
    usleep(117000);
    drive_speed(20);
