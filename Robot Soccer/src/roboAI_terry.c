 ...
 {            // if we choose penalty kick
              ...
              // check which side we are on.
                ...
                if (fabs(ai->st.self->cx[0] - ai->st.ball->cx[0]) < 200 ){
                  ...
                  fprintf(stderr, "distance : %f\n", fabs(ai->st.self->cx[0] - ai->st.ball->cx[0]));
                  fprintf(stderr, "mx : %f  my : %f \n", ai->st.self->mx,ai->st.self->my);
                  ...
                  double angle_out = angle_between(ai);
                  int left_right = perfect_kick(ai);
                  if (flr == 99){
                    flr = left_right;
                  }

                  double angle_inside = angle_in(ai);
                  int error = 5 - (int)angle_out;
                  fprintf(stderr, "off angle from center %f\n", fabs(angle_out));
                  fprintf(stderr, "perfect_kick value %d\n", left_right);
                  turn_bot(left_right, angle_out);
                  if (error < 10){
                    if (flr == left_right){
                    pivot_bot(left_right, angle_inside);
                    drive_speed(20);
                    usleep(100000);
             ...
             // bot is below the ball, NOT perpendicular to the field
                ...
                if (fabs(ai->st.self->cx[0] - ai->st.ball->cx[0]) < 200 ){
                  ...
                  if (error < 10){
                    if (flr == left_right){
                    turn_bot(left_right, angle_inside);
                    drive_speed(20);
                    usleep(100000);
...
double angle_in(struct RoboAI *ai){
  ...
  //magnitude of each vectors
  double mag_rxy = sqrt(pow(rx,2) + pow(ry,2));
  double mag_rbxy = sqrt(pow(perfect_line_x,2) + pow(perfect_line_y,2));
...
void turn_bot(int left_right, double angle){
  double error = angle - 5.0;
  if (left_right){
    turn_left_speed(2* (int)error);
    usleep(117000);
    drive_speed(20);
...
void pivot_bot(int left_right, double angle){
  double error = angle - 3.0;
  if (left_right){
    pivot_left_speed(2* (int)error);
    usleep(117000);
  }
  else{
    pivot_right_speed(2* (int)error);
    usleep(117000);
